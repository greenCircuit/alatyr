...
func buildPolicyStatus(policies []*istiosec.AuthorizationPolicy) models.PolicyStatus {
	...
	// HasL7: true when any selecting policy has any rule with L7 operation
	// fields (hosts/methods/paths or their notX exclusions). Computed once
	// up front because the deny-all / allow-all short-circuits below would
	// otherwise miss it.
	for _, authzPolicy := range policies {
