...
type Recorder struct {
	...
	// Section 3.5 — policy layering. Wide+narrow policy stack where the coarse
	// rule blocks and the fine rule allows verified paths. Split out of issues
	// because it's an expected multi-engine pattern, not a finding.
	policyLayering             *prometheus.GaugeVec
	policyLayeringByEngine     *prometheus.GaugeVec
	policyLayeringByPolicy     *prometheus.GaugeVec
	workloadsWithLayering      *prometheus.GaugeVec
	workloadsWithLayeringByType *prometheus.GaugeVec
...
func (r *Recorder) registerMetrics() {
	...
	// Section 3.5 — policy layering (wide+narrow expected pattern). Same shape
	// as alatyr_issues family but separate name so a dashboard cannot conflate
	// expected layering with real findings.
	r.policyLayering = factory.NewGaugeVec("policy_layering",
		"Expected wide+narrow policy layering per namespace per type. Coarse rule blocks, fine rule allows verified paths — not a finding.",
		"namespace", "type")
	r.policyLayeringByEngine = factory.NewGaugeVec("policy_layering_by_engine",
		"Policy-layering events attributed to the blocker engine.",
		"namespace", "type", "engine")
	r.workloadsWithLayering = factory.NewGaugeVec("workloads_with_policy_layering",
		"Distinct workloads sitting inside a wide+narrow layering pattern, per namespace.",
		"namespace")
	r.workloadsWithLayeringByType = factory.NewGaugeVec("workloads_with_policy_layering_by_type",
		"Distinct workloads inside layering of the given type, per namespace. Sum across types is NOT a workload count.",
		"namespace", "type")
	...
	if r.cfg.IssuePolicyBreakdownEnabled() {
		...
		r.policyLayeringByPolicy = factory.NewGaugeVec("policy_layering_by_policy",
			"Layering counts attributed to the blocker policy. Same gate as issues_by_policy.",
			"namespace", "type", "engine", "policy_namespace", "policy_name")
	}
