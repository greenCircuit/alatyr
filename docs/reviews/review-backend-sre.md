...
### 1. Workload-level ambient opt-in on non-ambient ns is silently dropped - FIXED
...
### 2. `IngressNamespace = "istio ingress"` typo (space, not hyphen) - FIXED
...
### 3. Ingress peer resolved via `DstID` not `SrcID` - FIXED
...
### 4. Dead `len(rule.Ports)==0` check inside port loop - FIXED
...
### 6. Root PA-fetch failure Issue lacks `Node` - FIXED
...
- Ambient workload label on a non-ambient ns — would catch P0#1. - FIXED (`TestBuildMeshMembership_PerWorkloadEnrollment`)
- `IngressNamespace` matches real `istio-ingress` — would catch P0#2. - FIXED (`TestSystemNamespaceConstants`)
- `ValidateExternalRules` ingress tests never exercise the k8s swap (`SrcID` is unset, `DstID` holds the peer). Add: `SrcID` = out-of-mesh peer, `DstID` = the workload under test, non-ambient `SrcNamespace` → assert not flagged. Catches P1#3. - FIXED (`allowRule` now direction-aware; `TestValidateExternalRules_OutOfMeshPeerNotFlagged` ingress cases)
- `BuildMeshMembership` has no test for `NSNode == nil`. Would panic on `idx.NSNode.Labels` at `buildMeshMembership.go:41`. Fetch pipeline guards it now, but a nil check + one-line test is cheap insurance. - FIXED (guard + `TestBuildMeshMembership_NilNSNode`)
