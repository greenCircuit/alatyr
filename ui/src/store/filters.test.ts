...
import { filteredNodes, filteredEdges, nodeMatchesMeshFilter, type FilterState, type MeshFilterValue } from './filters';
...
describe('filteredNodes', () => {
  ...
  describe('mesh filter', () => {
    ...
    // Namespace-type nodes are filtered by the SAME matcher (TablesView prepends
    // them separately). Regression guard: an out-of-mesh ns must NOT pass an
    // in-mesh/mtls selection, and an enrolled ns must pass its own verdict.
    describe('nodeMatchesMeshFilter (namespace rows)', () => {
      const inMeshNs: WorkloadNode  = { id: 'ns-a', label: 'a', namespace: '', type: 'namespace', labels: {} };
      const outMeshNs: WorkloadNode = { id: 'ns-b', label: 'b', namespace: '', type: 'namespace', labels: {} };
      const nsMesh: Record<string, MeshMembership> = {
        'ns-a': inMeshStrict,
        'ns-b': outOfMesh,
      };

      it('empty selection passes any namespace node', () => {
        expect(nodeMatchesMeshFilter(outMeshNs, nsMesh, new Set())).toBe(true);
      });

      it('in-mesh excludes out-of-mesh namespace, keeps enrolled', () => {
        const sel = new Set<MeshFilterValue>(['in-mesh']);
        expect(nodeMatchesMeshFilter(outMeshNs, nsMesh, sel)).toBe(false);
        expect(nodeMatchesMeshFilter(inMeshNs, nsMesh, sel)).toBe(true);
      });

      it('mtls verdict filters namespace nodes by their own verdict', () => {
        expect(nodeMatchesMeshFilter(inMeshNs, nsMesh, new Set<MeshFilterValue>(['mtls-strict']))).toBe(true);
        expect(nodeMatchesMeshFilter(inMeshNs, nsMesh, new Set<MeshFilterValue>(['mtls-disable']))).toBe(false);
        expect(nodeMatchesMeshFilter(outMeshNs, nsMesh, new Set<MeshFilterValue>(['mtls-strict']))).toBe(false);
      });
    });
  });
});
